...
class KITTI_Loader(data.Dataset):
    ...
    def __init__(
        ...
        # self, root, seed=None, train=0, sequence_length=3, transform=None, data_degradation=0, data_random=True)
        np.random.seed(seed)
        random.seed(seed)
        self.root = Path(root)
        train = 0
        if train == 0:
            ...
            # scene_list_path = self.root/'calib'
        if train == 1:
    ...
    def crawl_folders(self, sequence_length):
        ...
        for scene in self.scenes:
            ...
            if "garbage" in str(scene):
                imu_path = Path(os.path.dirname(scene).split("attack")[0]) + "/imu/"
                pose_path = Path(os.path.dirname(scene).split("attack")[0])
            else:
                imu_path = scene + "/imu/"
                pose_path = scene
            poses_l = (
                np.genfromtxt(pose_path / "poses.txt")
                .astype(np.float64)
                .reshape(-1, 3, 4)
            )
            imgs_l = sorted(scene.files("*.png"))[0 : len(poses_l)]
            imus_l = sorted(imu_path.files("*.txt"))[0 : len(poses_l)]

            # print("here in train 0 crawl ", len(imgs_l),len(imus_l))
            if len(imgs_l) < sequence_length:
                continue

            for i in range(
                demi_length, len(imgs_l) - demi_length - 10
            ):  # CHANGING -10 here
                ...
                for j in shifts:
                    ...
                    sample["imus"].append(
                        np.genfromtxt(imus_l[i + j]).astype(np.float32).reshape(-1, 6)
                    )
    ...
    def crawl_test_folders(self):
        ...
        for scene in self.scenes:
            ...
            poses_l = (
                np.genfromtxt(scene / "poses.txt").astype(np.float64).reshape(-1, 3, 4)
            )
            ...
            for i in range(len(imgs_l)):
                ...
                sample["imus"].append(
                    np.genfromtxt(imus_l[i]).astype(np.float32).reshape(-1, 6)
                )
    ...
    def __getitem__(self, index):
        ...
        imgs = [
            load_as_float(img, data_degradation[n_img])
